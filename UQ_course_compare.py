...
def parse_UQ_course_list(content):
    hrefs = []  # 存放抓取的链接
    course_list = {}
...
def parse_UQ_course_details(content):
    html = etree.HTML(content)
    href = html.xpath("//table[@id='course-archived-offerings']/tbody/tr[1]/td[4]/a/@href")
    return href


def get_course_profile_url_list(part_of_hrefs):
    course_profile_urls = []
    for _ in part_of_hrefs:
        for href in _:
...
liuyue_url = "https://my.uq.edu.au/programs-courses/program_list.html?acad_prog=5660&year=2020"   #刘月
zhuxuyang_url = "https://my.uq.edu.au/programs-courses/program_list.html?acad_prog=5581"          #朱旭阳
my_url = "https://my.uq.edu.au/programs-courses/program_list.html?acad_prog=5521&year=2020"       #李嘉辉
headers = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36'
}
liuyue_page = get_url(liuyue_url,headers)
zhuxuyang_page = get_url(zhuxuyang_url,headers)
my_page = get_url(my_url,headers)

lol1, liuyue_course_list = parse_UQ_course_list(liuyue_page)
lol2, zhuxuyang_course_list = parse_UQ_course_list(zhuxuyang_page)
lol3, my_course_list =  parse_UQ_course_list(my_page)

liuyue_coursecode_list = list(liuyue_course_list.keys())
zhuxuyang_coursecode_list = list(zhuxuyang_course_list.keys())
my_coursecode_list = list(my_course_list.keys())


me_and_zhu = [i for i in zhuxuyang_coursecode_list if i in my_coursecode_list]

same = [i for i in me_and_zhu if i in liuyue_coursecode_list]
